...
void Timer1A_Init(void)
{
    ...
    NVIC->IP[21] = 2 << 5; // 19: IRQ number; 2: priority; 5: fixed-shift 5 position
    NVIC->ISER[0] = 1<<21; // enable interrupt 19 in NVIC
    __enable_irq(); /* global enable IRQs */
}
...
{
	TIMER0->ICR |=0x4;//clear timer0A flag (icr)
	turnOn();//turn on LED
	TIMER0->IMR &=~0x4;//disarm timer0a
	TIMER0->CTL &=~0x1;//disable timeroa	
	TIMER1->IMR |=0x4;//arm timer1A(imr)
	TIMER1->CTL |=0x1;//enable timer1A(ctl)
	time = TIMER1->TAR;
}

void TIMER1A_Handler (void)
{
	TIMER1->ICR |=0x4;//clear timer1a flag
	time=(time-TIMER1->TAR)&0x00FFFFFF;//time=(time-timer1a->tar)&0x00ffffff
	turnOff();//turn off led
	updateFlag=1;//update flag =1
	TIMER1->IMR &=~0x4;//disarm timer1a
	TIMER1->CTL &=~0x1;//disable timer1a
	TIMER0->IMR |=0x4;//arm timer01
